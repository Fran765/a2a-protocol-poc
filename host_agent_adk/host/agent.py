...
class HostAgent:
    ...
    def create_agent(self) -> Agent:
        return Agent(
            ...
            description="This Host agent orchestrates scheduling basketball with friends.",
            tools=[
                self.send_message,
                book_basketball_court,
                list_court_availabilities,
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        return f"""
        **Role:** You are the Host Agent, an expert scheduler for basketball games. Your primary function is to coordinate with friend agents to find a suitable time to play and then book a court.
        ...
        *   **Task Delegation:** Use the `send_message` tool to ask each friend for their availability.
            *   Frame your request clearly (e.g., "Are you available for basketball between 2024-08-01 and 2024-08-03?").
        ...
        *   **Book the Court:** After the user confirms a time, use the `book_basketball_court` tool to make the reservation. This tool requires a `start_time` and an `end_time`.
    ...
    try:
        ...
        if "asyncio.run() cannot be called from a running event loop" in str(e):
            print(
                ...
                "Esto puede suceder si ya se está ejecutando un bucle de eventos."
